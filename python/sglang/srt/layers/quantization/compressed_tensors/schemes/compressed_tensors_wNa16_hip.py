...
class CompressedTensorsWNA16HIP(CompressedTensorsLinearScheme):
    ...
    def apply_weights(self, layer, x, bias=None):
        ...
        import os
        if os.environ.get("SGLANG_DEBUG_WNA16HIP"):
            print(f"[WNA16HIP] apply_weights x.dtype={x.dtype} x.shape={tuple(x.shape)} N={N} w_q.shape={tuple(w_q.shape)} w_s.dtype={w_s.dtype} M={M}", flush=True)
